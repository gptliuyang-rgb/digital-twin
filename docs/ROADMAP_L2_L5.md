...
工业场景里箱子会“搬起来、码上去、枪对准二维码”，默认走 `mj_step` + 重力补偿 PD + 约束焊
（ADR-007 物理路径）。这仍不是手–箱 Coulomb 力封闭；L2.3 几何门（`scan_geometry_ok`）继续用
`mj_forward` 运动学回放。`grasp_success_rate` 在 live E1/E2 之前禁止写。
