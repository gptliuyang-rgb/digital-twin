...
- **Consequences:** `make eval-l1a-pd-plant` dumps delayed a-history vs PD q_des vs τ. After 10 ticks with `q[0]=i` and `a_t[0]=10+i`, decoder `a[0]` is `[0, 10..18]`, PD `q_des[0]` is 19, and `τ[0] = 1080 × (19 − 9) = 10800 N·m`. One 50 Hz command occupies 10 identical 500 Hz torque samples. Do not treat this plant as a trained decoder ONNX run or as SONIC tracking PD. A live T800 policy output is still missing — the caller must supply `policy_action`. Closing the 500 Hz plant onto a joint-PD command plant *inside MuJoCo on this same tick* (without treating τ as an ONNX run) is ADR-056.

## ADR-056 — Held 25-D PD τ is applied for 10 physics substeps on the same tick

- **Status:** accepted
- **Context:** Official `g1_deploy_onnx_ref.cpp` applies joint PD at 500 Hz between 50 Hz gathers. ADR-055 evaluated `τ = Kp(a_t − q) − Kd q̇` from the 50 Hz HardwareSnapshot but did not step a plant. Interpolating that snapshot onto 500 Hz, finite-differencing dq, rewriting this tick's decoder 874-D, or inventing IMU from undocumented body rates would not match the four-rate stack. Loading G1 `model_decoder.onnx`, welding DexHand2, or treating τ as an ONNX output remain refused. `wbc/` must stay numpy-only (no MuJoCo import). EngineAI `pd_stand` is still bring-up, not SONIC tracking (ADR-022). Table S4 and pad–cardboard are unrelated.
- **Options:** (A) leave mj_step to `T800MujocoEnv.step` callers until a T800 decoder ONNX exists; (B) attach an optional `JointPdPhysics` backend on `SharedPlaybackCursor`: after ADR-055, run 10 substeps at 1/500 s with measured q/dq, push joints (not IMU) for the *next* gather, refuse G1 29-D / 32-D qpos / 45-D hands / 75-D command_schema / fake ONNX / Hermite / finite-diff / this-tick decoder rewrite / invented IMU / non-0.002 s dt; (C) import MuJoCo into `wbc/` and spline 29-D G1 τ.
- **Decision:** (B). `wbc/pd_physics.py` stays shared sim/real (no MuJoCo/Isaac/PICO/onnxruntime). `T800MujocoEnv.apply_tau_and_step` is the MuJoCo backend. Omit the backend to keep ADR-055. `pd_tau_nm` stays the 50 Hz snapshot; `pd_physics_tau_nm` is the last 500 Hz sample. ADR-018 interpolators stay the runtime L1a. `grasp_success_rate` stays JSON `null`. Combined T800+Hand stays `PolicyEvalBlocked`.
- **Consequences:** `make eval-l1a-pd-physics` dumps delayed a-history vs pre-physics q vs 10 substeps. After 10 ticks with `q[0]=i` and `a_t[0]=10+i`, decoder `q[0]` is still `[0..9]` (push_hw before assemble) while physics q[0] is `9.01` on the increment double. Do not treat this plant as a trained decoder ONNX run or as SONIC tracking PD. A live T800 policy output is still missing — the caller must supply `policy_action`. A real IMU model is still missing — do not invent one from physics. Closing the next gather onto those joints when the caller omits `push_hw` is ADR-057.

## ADR-057 — Omit push_hw so next-tick decoder q follows the 500 Hz plant

- **Status:** accepted
- **Context:** Official `g1_deploy_onnx_ref.cpp` gathers IMU + joints after 500 Hz PD has already moved the robot. ADR-056 applied τ for 10 substeps and copied q/dq into `HardwareHold` for the *next* gather, but every eval still called `push_hw` each tick and overwrote the plant. That is the HIL path (measured joints). The sim path must be able to omit `push_hw` so decoder q follows the plant. Inventing IMU from undocumented body rates, rewriting this tick's 874-D, finite-differencing dq, or loading G1 `model_decoder.onnx` remain refused. EngineAI `pd_stand` is still bring-up, not SONIC tracking (ADR-022). Table S4 and pad–cardboard are unrelated.
- **Options:** (A) keep requiring `push_hw` every tick until a T800 decoder ONNX exists; (B) document and test omit-`push_hw`: tick 0 still needs a full snapshot, later ticks assemble from post-physics q/dq while keeping IMU, `push_hw` still overrides, refuse invented IMU / this-tick rewrite / Hermite / G1 29-D / hands; (C) copy MuJoCo body rates into IMU and rewrite this tick's decoder.
- **Decision:** (B). `HardwareHold.push_joints` already copies q/dq only. `SharedPlaybackCursor` does not invent IMU. ADR-018 interpolators stay the runtime L1a. `grasp_success_rate` stays JSON `null`. Combined T800+Hand stays `PolicyEvalBlocked`.
- **Consequences:** `make eval-l1a-pd-closedloop` dumps delayed a-history vs plant-following q vs kept IMU. After 10 ticks on the increment double with first-tick `q[0]=0` and `a_t[0]=10+i`, decoder `q[0]` is `[0.00, 0.01, …, 0.09]`, physics q[0] is `0.10`, `τ[0] = 1080 × (19 − 0.09)`, and decoder ω stays the first snapshot. Do not treat this as a trained decoder ONNX run or as SONIC tracking PD. A live T800 policy output is still missing — the caller must supply `policy_action`. A measured IMU model is still missing — do not invent one from physics. Wiring `command_schema` finger q onto the 1 kHz MIT ring (still bypassing WBC) is ADR-058.

## ADR-058 — DexHand2 1 kHz MIT ring, not inside WBC last_action

- **Status:** accepted
- **Context:** Official Wuji Hand 2 is MIT force-position hybrid at 1000 Hz × 20 (`τ = kp (qd − q) + kd (dqd − dq) + τ_ff`). `command_schema_v1` already splits `hand_slice` from the 32-D WBC pose block. ADR-054–057 ZOH-hold T800 25-D `policy_action` onto a 500 Hz PD plant. Concatenating DexHand2 20-D onto that 25-D (45-D), copying the 75-D flat command into last_action, using `hardware_kp` (REQUIRED_INPUT), inventing `command_latency_ms`, or treating MJCF `forcerange` as a payload rating would mix the two rings. `wbc/*.yaml` keep `not_hand_mit_ring: true`. Table S4 and pad–cardboard are unrelated. Combined T800+Hand stays `PolicyEvalBlocked`.
- **Options:** (A) leave fingers to `DexHand2Controller.set_joint_targets` one-shot until hardware kp is filled; (B) add a 1 kHz ZOH ring in `hand/` + `runtime/hand_bypass.py` that consumes `left_hand_q` / `right_hand_q`, uses MJCF gen-1 `sim_kp`/`sim_kv` labelled `mjcf_gen1_carryover_not_hand2_sysid`, clips τ to MJCF forcerange, refuses 25/29/32/45/75-D, refuses Hermite / finite-diff / invented latency / force-mode-without-E3, does not write WBC last_action; (C) concatenate 20-D hand q into decoder last_action and import MuJoCo into `wbc/`.
- **Decision:** (B). `hand/mit_ring.py` / `runtime/hand_bypass.py` stay shared sim/real (no MuJoCo/Isaac/PICO/onnxruntime). `wbc/` is unchanged: `not_hand_mit_ring: true`. `grasp_success_rate` stays JSON `null`. Combined T800+Hand stays `PolicyEvalBlocked`.
- **Consequences:** `make eval-l1c-mit-ring` dumps 10 ticks × 20 substeps. After 10 ticks on the increment double with `q_des[0]=0.5`, hand q[0] is `0.20` and last τ is `kp0 × (0.5 − 0.199)`. Do not treat this as Hand 2 sys-id or as a decoder ONNX run. Hardware kp/kd, command latency, and E3 torque stay REQUIRED_INPUT. Force mode stays refused. A live T800 policy output is still missing on the body path — the caller must supply `policy_action`. Closing that ring onto a 1 kHz MuJoCo plant (still no T800 weld) is ADR-059.

## ADR-059 — DexHand2 1 kHz MuJoCo HandMitPhysics, not a T800 weld

- **Status:** accepted
- **Context:** ADR-058 ZOH-holds `left_hand_q` / `right_hand_q` onto a 1 kHz MIT ring but stepped a numpy test double. Official Hand 2 MJCF uses `<position>` actuators with gen-1 kp/kv baked in; writing MIT τ onto those ctrl slots would not be the hybrid law. Concatenating DexHand2 onto T800, resampling onto the body 500 Hz `dt`, using `hardware_kp` (REQUIRED_INPUT), inventing `command_latency_ms`, or treating MJCF `forcerange` as a payload rating would mix the two rings. `wbc/` must stay numpy-only. Table S4 and pad–cardboard are unrelated. Combined T800+Hand stays `PolicyEvalBlocked`.
- **Options:** (A) leave the plant as the numpy increment double until hardware kp is filled; (B) add `HandMitMujocoEnv` in `sim/` that implements `HandMitPhysics`: 20-D `<motor>` fixture at 1/1000 s (placeholder inertias, gravity off, no contact), optional derived `*_mit.xml` when cloned, refuse official `<position>` / G1 29-D / T800 25-D / 45-D concat / 75-D command_schema / 0.002 s dt / T800 weld / finite-diff dq; (C) import MuJoCo into `wbc/` and weld the hand onto `serial_t800.xml`.
- **Decision:** (B). `sim/mujoco_env/hand_mit_physics.py` is the MuJoCo backend. `hand/mit_ring.py` / `runtime/hand_bypass.py` stay shared sim/real (no MuJoCo/Isaac/PICO/onnxruntime). `wbc/` is unchanged: `not_hand_mit_ring: true`. Fixture inertias are placeholders, not CAD. `grasp_success_rate` stays JSON `null`. Combined T800+Hand stays `PolicyEvalBlocked`.
- **Consequences:** `make eval-l1c-mit-physics` dumps 10 ticks × 20 substeps on the fixture. After 10 ticks with `q_des[0]=0.5`, measured `q[0]` moves toward the hold. Without MuJoCo the eval reports `runtime=unavailable`. Do not treat this as Hand 2 sys-id, a pad–cardboard number, or a decoder ONNX run. Hardware kp/kd, command latency, and E3 torque stay REQUIRED_INPUT. Force mode stays refused. A live T800 policy output is still missing on the body path — the caller must supply `policy_action`. Closing that plant onto official skeleton inertias (still no `<position>` XML, still no T800 weld) is ADR-060.

## ADR-060 — Official-inertia 1 kHz MIT plant, not `<position>` XML

- **Status:** accepted
- **Context:** ADR-059 stepped the MIT ring on a 20-D `<motor>` fixture whose masses (0.01 kg) and inertias (1e-6 kg·m²) are placeholders. Official Hand 2 MJCF has CAD `<inertial>` tags (skeleton 0.6207 kg) but ships `<position>` actuators and `timestep=0.002`. Loading that file as-is would bake gen-1 kp/kv into the plant and resample the 1 kHz ring onto 500 Hz. `source=auto` previously selected `official_position` once `wuji-description` was cloned, which then raised. Treating fixture inertias as CAD, welding T800, inventing soft-body CoM, or using `hardware_kp` remain refused.
- **Options:** (A) keep the fixture until `hardware_kp` is filled; (B) rewrite official MJCF into a mesh-free `<motor>` plant that keeps CAD inertias, strips meshes/contact, forces `dt=0.001` and gravity off, auto-select that when cloned, refuse unconverted `<position>` / 0.002 s / G1 29-D / T800 weld / fixture-as-CAD; (C) load official `<position>` XML and write MIT τ onto those ctrl slots.
- **Decision:** (B). `to_meshfree_mit` in `assets/dexhand2/build/gen_derived.py`; `HandMitMujocoEnv(source="official_inertia")` is the backend. `hand/mit_ring.py` / `runtime/` / `wbc/` stay numpy-only. Soft-body mass is still not in the plant (`com_in_wrist_frame_m` REQUIRED_INPUT). `grasp_success_rate` stays JSON `null`. Combined T800+Hand stays `PolicyEvalBlocked`.
- **Consequences:** `make eval-l1c-mit-physics` dumps 10 ticks on official inertias when cloned (else fixture). Plant mass matches skeleton 0.6207 kg. Fixture q[0] after 10 ticks differs from official. Do not treat this as Hand 2 sys-id, a pad–cardboard number, or a decoder ONNX run. Hardware kp/kd, command latency, and E3 torque stay REQUIRED_INPUT.
