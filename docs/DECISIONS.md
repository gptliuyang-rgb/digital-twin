...
- **Consequences:** `make eval-l2-freebase-push` adds 6 CoM-hold cases. A change in lean vs fall across CoM offset is a diagnostic, **not** a SONIC fail and **not** a wrist hang-test. Restitution and `default_joint_pos_offset_rad` stay unswept. PPO / GMR-on-BONES-SEED / combined weld remain blocked on flange SE(3) and wrist CoM.
