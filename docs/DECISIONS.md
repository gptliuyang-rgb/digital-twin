...
- **Consequences:** `make eval-l1a-decoder-tick` dumps 874-D vs encoder look-ahead. After 10 ticks, decoder `q[0]` history is the hardware sequence; encoder 10frame_step5 stays on clip indices. `play == false` holds `current_frame` but still logs hardware. Do not treat this wiring as a trained decoder ONNX run.
