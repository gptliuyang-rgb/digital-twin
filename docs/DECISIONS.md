...
- **Consequences:** `make eval-l1a-playback` dumps the clamp table. A successful first assign of a 16-frame clip with `play` leaves `current_frame == 1` on that same tick. One-frame clips clamp (and may readapt) on the first tick. Do not treat this cursor as a trained planner.
