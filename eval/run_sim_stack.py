...
(ADR-004). Identity flange (ADR-006). L2.3 industrial is kinematic geometry
(ADR-007); physics industrial is mj_step + PD + optional welds.
"""
...
def _markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "L2.3 industrial carton/gun motion is kinematic playback (ADR-007). "
        "Viewer/GIF default to mj_step + gravity-comp PD + constraint welds.",
        "",
        f"- **ok:** `{report.get('ok')}`",
    ...
    lines.extend(
        [
            ...
            "- Physics industrial welds are constraint grasps, not E1/E2. "
            "`use_welds=false` still drops the box; that is honesty, not a task pass.",
            "",
        ]
    )
    return "\n".join(lines) + "\n"
