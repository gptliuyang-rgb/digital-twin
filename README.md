...
This repository is the **digital-twin layer**: frozen command contracts, official-asset ingest, a sim/real shared hand controller, L0–L2 eval, and a **simulation-only** T800 + DexHand2 assembly for scripted industrial strategies (dual-arm box carry / stack / handheld QR scan). It does **not** train SONIC or a VLA, and it does **not** report grasp success until contact parameters are measured.

Policy client code under `runtime/` and `vla/client/` does not import MuJoCo or Isaac. Only `sim/*` / `hand/backends/mujoco_backend.py` talk to a simulator.

## Layout

```
interface/     command_schema_v1.yaml, frames.yaml, schema.py
assets/        dexhand2 spec + official ingest; T800 paths; combined T800+hands assemble
...
A combined MJCF/URDF is generated with an **identity kinematic-bringup weld** (ADR-006). That is enough to run the scripted twin in MuJoCo; it is **not** a CAD flange and must not be used for policy-eval numbers.
